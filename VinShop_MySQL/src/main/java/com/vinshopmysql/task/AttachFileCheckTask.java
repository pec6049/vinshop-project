...
public class AttachFileCheckTask {
	...
	public void checkFiles() throws Exception {
		...
		//썸네일 이미지
		fileList.forEach(vo -> {
			Path path = Paths.get("C:\\upload\\temp", vo.getUploadPath(), "s_" +  vo.getUuid() + "_" + vo.getFileName());
			checkFilePath.add(path);
		});
		
		// 디렉토리 파일 리스트
		File targetDir = Paths.get("C:\\upload\\temp", getFolderYesterDay()).toFile();
